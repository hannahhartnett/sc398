...
shinyApp(
  ...
  tags$head(
    tags$style("label{font-family: Palatino;}")
  ),
  inputPanel(
...
server <- shinyServer(function(input, output) {
  ...
  scale_color_manual(values='blue')  +
      theme(axis.text.x = element_text(angle = 35, hjust = 1, vjust = 0.5, size = 5)), 
      tooltip = 'y')
  })
  })
...
server <- shinyServer(function(input, output) {
  ...
  output$actorPlot2 <- renderPlotly({
    p3 <- ggplot(acled %>% filter(assoc_actor_label1 == input$actor2), 
      ...
      theme(axis.text.x = element_text(angle = 35, hjust = 1, vjust = 0.5)) + plot_theme
...
>>>>>>> a76c7e7b64783cd7d9f883ce6ea398c5dca29a81
  })
})
...
deaths_by_month <- 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 0.5)) + plot_theme
ggplotly(deaths_by_month, tooltip = c("PctTot"))
    })
  }, 
    options = list(height = 479)
)
```
