...
plot_theme <-  theme_gray() + # White background, black and white theme
  theme(axis.text = element_text(size = 10, color = "azure4"),
        text = element_text(size = 10, face = "bold", color = "azure4", family = 'News Cycle'))
```
  ...
  server = function(input, output) {
    output$event_map <- renderLeaflet({
      ...
      output$sub_eventPlot <- renderPlotly({
        ggplotly(ggplot(acled %>% filter(event_type == input$event & (event_date <= input$slider[2] & event_date >= input$slider[1])), aes(x = sub_event_type)) + geom_bar() +
...
shinyApp(ui = ui, server = server, options = list(height = 725))
```
